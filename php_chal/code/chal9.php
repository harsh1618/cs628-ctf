<?php
  ...
  if(!isset($_SESSION['username'])) {
    ...
    if(array_key_exists("username", $_POST)) {
      $db = new PDO('mysql:host=db;dbname=chal9', 'chal9', 'randomdifficultpassword');
      $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      try {
        $date = date("d H:i:s", time() + (330 * 60));
        file_put_contents('/var/www/html/sql9.log', $_SESSION['username'] . " " . $date . " " . $_POST['username'] . "\n", FILE_APPEND);
    ...
    <div class="card-panel" style="margin-top:5em; height: 20em; ">
      ...
      <div class = "row" >
        <div class="col s2" style="float:right">
          <a href="source9.html" class="waves-effect waves-light btn-large">View Source</a>
        </div>
      </div>
    </div>
  </div>
